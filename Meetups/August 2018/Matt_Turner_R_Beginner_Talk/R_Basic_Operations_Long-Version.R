# R Basic Operations (Long Version)
#
# Matthew D. Turner
# mturner46@gsu.edu
...
#
# Here is how to add the "loess" smoother to the same figure:

lom <- loess(hwy ~ displ, data = mpg)    # Build the loess model
xax <- seq(1, 7, by = 0.1)               # Get values on x-axis

# What does xax look like?
xax

# Get corresponding y values (the predict function gets these!):
predict(lom, xax)

lines(xax, predict(lom, xax), lwd = 3, col = "green")
...
# Compare the previous two examples for loess and the linear model.
# Both of these follow the same general format: plot the points, fit a
# model, call an appropriate plotting function to show the model on
# the data.
...
# The following part is a bit more advanced!!
# 
# Note that R refuses to show us the full list of manufacturers. This
# is reasonable, as there are 234 of these. But what if we want to see
# them all?
#
# R is very type-driven and polymorphic. Objects tend to have at least
# one class/type associated with them that drives R's behavioral
# choices.

x <- mpg[,1]  # Put the manufacturer column in x

dim(x)        # 234 total items
table(x)      # Break out by manufacturer (works right!)

str(x)        # Show us the types of x

# R is *extremely* polymorphic. The print method has rules for how to
# show us data frames and 'tbl' objects (tables/tibbles) we need to
# override this behavior to see all the data:

print(x)           # Follow the rules for the type
print(as.list(x))  # Force R to treat x as a different type

# Back to what we were doing:

mpg[1:10,] # Data for first 10 "subjects" (here cars)

table(mpg$manufacturer)   # How many Audis? Can we grab them?
...
temporaryVariable <- mpg$manufacturer == "audi"
newVersion <- mpg[temporaryVariable,]
...
# (Sort of like a "closure" in computer science.)
#
# Some plots:
...
## 6: New Topic -- File and Directory Manipulation
#
# Everything you can do in R, you can do with commands at the command
# line. While RStudio lets you change directories, open files, import
# data, etc., it may be necessary for you to **program** this
# behavior. For that you need commands like these:

getwd()            # Where are you? Current working directory
list.files()       # What files are there?
x = list.files()   # Store files in a variable
x[3]
